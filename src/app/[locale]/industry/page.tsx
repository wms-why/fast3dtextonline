...
import { Badge, Box, Button, Card, Flex, Grid, Heading, Section, Text } from "@radix-ui/themes";
...
export default function IndustryListPage() {
  ...
  return (
    <Flex direction={"column"} gap={"4"}>
      <Header />
      <Section py="8" style={{ background: "radial-gradient(circle at top left, rgba(255,121,178,0.14), transparent 25%), radial-gradient(circle at top right, rgba(98,212,255,0.14), transparent 25%)" }}>
        <Flex direction="column" gap="8" px="6" mx="auto" style={{ maxWidth: 1240 }}>
          ...
          <Box>
            ...
            <Grid columns={{ initial: "1", md: "2", lg: "3" }} gap="5">
              {industries.map((i) => {
                const primary = getStylePreset(i.styleSlugs[0]);
                const editorHref = primary ? getEditorPath(primary.editorPreset, locale) : `/${locale}/editor`;
                return (
                  <Card key={i.slug} size="3" style={{ borderRadius: 24 }}>
                    <Flex direction="column" gap="3">
                      <Flex justify="between" align="center" wrap="wrap">
                        <Badge radius="full" color="iris" size="2">{i.displayName}</Badge>
                        <Text size="1" color="gray">{t("sampleTextLabel")}: <strong>{i.sampleText}</strong></Text>
                      </Flex>
                      <Heading as="h3" size="5">
                        <a href={`/${locale}/industry/${i.slug}`} style={{ color: "inherit", textDecoration: "none" }}>
                          {locale === "zh" ? i.zh.title : i.en.title}
                        </a>
                      </Heading>
                      <Text color="gray" size="2">{locale === "zh" ? i.zh.summary : i.en.summary}</Text>
                      <Flex gap="2" wrap="wrap">
                        {i.keywords.slice(0, 3).map((kw) => (
                      ...
                      <Flex gap="2" mt="auto" wrap="wrap">
                        ...
                        <Button asChild radius="full" size="2" variant="soft"><a href={`/${locale}/industry/${i.slug}`}>{t("seeDetails")}</a></Button>
                      </Flex>
                    </Flex>
                  </Card>
                );
