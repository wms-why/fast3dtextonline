'use client'
import { containsChinese, DefaultFontChinese, FontLang, Fonts, FontWeight, getFontPath, getFontWeight } from "@/lib/presets/fonts";
...
export default function TextSetting({
  ...
  const initTextColorMode = useRef(false);
  useEffect(() => {
  ...
  const initTextColor = useRef(false);
  useEffect(() => {
  ...
  const initTextGradientColor = useRef(false);
  useEffect(() => {
  ...
  const initTextGradientColorDir = useRef(false);
  useEffect(() => {
  ...
  const handleSelectFont = (font: string) => {
    ...
    if (f) {
      ...
      const f = uploadFonts.find((item) => item.name === font)!;
