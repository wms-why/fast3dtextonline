...
import type { ColorGradientDir, TextProp } from "./TextSetting";
...
export async function updateTextProp(textProps: TextProp) {
  ...
  if (lastTextProps == null) {
    ...
    const textMesh1 = new THREE.Mesh(geo, mat);
  ...
  scene.add(textMesh);
  if (outlineMesh) scene.add(outlineMesh);
  lastTextProps = textProps;
}
...
async function getTextGeometry(textProps: TextProp) {
  ...
  const font = await loadFont(textProps);
  const depth = 20,
    size = 70,
    // hover = 30,
    curveSegments = 4,
...
async function loadFont(textProps: TextProp) {
  ...
  const font = await loader.loadAsync(textProps.fontUrl);
  return font;
}
...
export function getPicture(width: number, height: number) {
  ...
  const lastWidth = renderer.domElement.width;
  const lastHeight = renderer.domElement.height;
  renderer.setSize(width, height, false);
  render();
