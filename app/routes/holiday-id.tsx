// /holiday/:holiday — holiday detail page.
import { Box, Button, Flex, Heading, Section, Text } from "@radix-ui/themes";
import { useLocale } from "@/lib/i18n/use-locale";
import { useTranslations } from "@/lib/i18n/use-translations";
import { getHoliday, getLocalizedHoliday } from "@/lib/presets/holiday-presets";
...
import type { Route } from "./+types/holiday-id";

export function meta({ params, location }: Route.MetaArgs) {
  const slug = params.holiday ?? "";
  const preset = getHoliday(slug);
  if (!preset) return [{ title: "Holiday not found" }, { name: "robots", content: "noindex, follow" }];
  const locale = (location.pathname.startsWith("/zh") ? "zh" : "en") as Locale;
  const copy = getLocalizedHoliday(preset, locale);
  return buildSeoMeta({
    title: copy.title,
...
export default function HolidayDetailPage({ params }: Route.ComponentProps) {
  const slug = params.holiday ?? "";
  const preset = getHoliday(slug);
  ...
  return (
    <Flex direction="column">
      <Section className="w-full bg-gradient-to-b from-red-50 to-white py-12 dark:from-red-950/30 dark:to-gray-950">
        <Box className="mx-auto w-full max-w-[1080px] px-6 text-center">
          ...
          <Flex gap="3" justify="center" wrap="wrap">
            ...
            <Button asChild radius="full" size="4" variant="soft">
              <LocaleLink to="/holiday">{t("backToList")}</LocaleLink>
            </Button>
          </Flex>
        </Box>
      </Section>
      ...
      <JsonLd
        data={generateCreativeWorkJsonLd({
          name: copy.title,
          description: copy.description,
          url: `${host}/${locale}/holiday/${preset.slug}`,
          keywords: preset.keywords,
          category: "3D Holiday Text",
        })}
      />
    </Flex>
  );
